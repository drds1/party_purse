...
class PartyPurseFlow(FlowSpec):
    """
    Metaflow ETL pipeline for UK political party funding data.
    
    Extracts data from Electoral Commission, structures with LLM,
    validates with Pydantic, and loads to Snowflake.
    """
    ...
    force_refresh = Parameter(
        "force-refresh",
        help="Force refresh of all data even if cached",
        default=False
    )
    ...
    def extract(self):
        ...
        try:
            ...
            # Store metadata
            self.extract_stats = {
                "records_fetched": len(self.raw_data) if isinstance(self.raw_data, list) else 1,
    ...
    def transform(self):
        ...
        try:
            ...
            self.transform_stats = {
                ...
                "parties_covered": list(set(f.party for f in self.structured_data.funders)),
            ...
            logger.info(f"✅ Extracted {self.transform_stats['funders_extracted']} funder records")
            logger.info(f"💰 Total tracked donations: £{self.transform_stats['total_amount']:,}")
    ...
    def load(self):
        ...
        try:
            ...
            logger.info(f"✅ Loaded {self.load_stats['records_loaded']} records to Snowflake")
            ...
            for party in self.parties_list:
                ...
                if not party_data.empty:
                    ...
                    if len(party_data) > 1:
                        ...
                        print(f"       £{second['amount_gbp']:,} ({second['percentage']}%)")
        
        print("\n" + "="*70)
        
        # Persist results as Metaflow artifacts for later inspection
        self.funders_list = [f.dict() for f in self.structured_data.funders] if hasattr(self, 'structured_data') else []
